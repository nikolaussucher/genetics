...
\chapter*{Cloning and Sequencing}\label{cloning-and-sequencing}
\addcontentsline{toc}{chapter}{Cloning and Sequencing}
...
\chapter{DNA extraction and GAPDH Polymerase Chain
Reaction}\label{dna-extraction-and-gapdh-polymerase-chain-reaction}
...
\item
  The PCR reaction will run for the next several hours using the
  following program:
\end{enumerate}
...
\item
  ...
  next several hours using the following Nested GAPDH PCR program:
\end{enumerate}

Initial denaturation: 95°C for 5 Then 40 cycles of: Denaturation: 95°C
for 1 Annealing: 52°C for 1 Extension: 72°C for 2 Final extension: 72°C
for 6 Hold: 15°C hold
...
\item
  Add 100 ml of 1x TAE (Tris base, acetic acid, EDTA) running buffer.
\item
  Add 10 µl of SybrGreen™ dye (10,000x stock solution).
\item
...
\item
  ...
  6x loading dye and stain. A smaller volume of the nested PCR is loaded
...
\section{Experimental Procedureqs}\label{experimental-procedureqs}
...
\midrule
\endhead
10x BgI II reaction buffer & 2 µl\tabularnewline
...
the bottom right well is H12. 1. Choose two of your plasmid minipreps to
sequence. 2. Plan your experiment. You will prepare four sequencing
samples for each of your two plasmid minipreps. You will combine each
plasmid miniprep with each of the four different sequencing primers ---
two forward primers and two reverse primers --- to ensure complete
coverage of the insert. pJET SEQ F and pJET SEQ R anneal to the pJET1.2
vector, while GAP SEQ F and GAP SEQ R are degenerate primers homologous
to internal GAPC sequences. In the table below, write which plasmid was
combined with which primer and added into each well. When you name your
sequences (sequence wells), make sure you use the same names when
submitting samples to the sequencing facility.
...
\tightlist
\item
  Label your microcentrifuge tubes for the well into which the reaction
  will be placed.
\item
